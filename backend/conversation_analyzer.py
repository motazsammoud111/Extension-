...
Supporte : WhatsApp (.txt), Telegram (JSON), Instagram (JSON)
"""
...
class WhatsAppParser:
    """
    Parse les exports WhatsApp (.txt).
    Formats supportés :
      - [DD/MM/YYYY, HH:MM:SS] Sender: message
      - DD/MM/YYYY, HH:MM - Sender: message
      - [DD.MM.YYYY, HH:MM:SS] Sender: message  (certains pays)
    """

    # Regex couvrant les variantes d'export WhatsApp
    PATTERNS = [
        # [12/03/2024, 14:30:55] Motaz: Salut !
        re.compile(
            r"^\[(\d{1,2}[/\-\.]\d{1,2}[/\-\.]\d{2,4}),?\s+"
            r"(\d{1,2}:\d{2}(?::\d{2})?(?:\s?[AP]M)?)\]\s+"
            r"([^:]+):\s(.+)$"
        ),
        # 12/03/2024, 14:30 - Motaz: Salut !
        re.compile(
            r"^(\d{1,2}[/\-\.]\d{1,2}[/\-\.]\d{2,4}),\s+"
    ...
    def parse(self, file_path: Path, my_name: str) -> Tuple[List[Message], List[Message]]:
        """
        Retourne (mes_messages, tous_les_messages).
        my_name : ton nom tel qu'il apparaît dans l'export (ex: "Motaz")
        """
        ...
        for line in lines:
            ...
            if not matched and current_msg:
                # Continuation du message précédent (multi-ligne)
                current_msg["text"] += "\n" + line.strip()
    ...
    def _finalize(self, raw: dict, my_name: str) -> Optional[Message]:
        ...
        # Filtrer les messages système et médias
        if any(s in text.lower() for s in self.SYSTEM_MESSAGES):
            return None
...
class TelegramParser:
    """
    Parse les exports Telegram (result.json).
    Structure : { "messages": [ { "from": str, "text": str, "date": str, ... } ] }
    """
    ...
    def parse(self, file_path: Path, my_name: str) -> Tuple[List[Message], List[Message]]:
        ...
        for m in messages:
            ...
            # Le texte peut être une liste de segments
...
class InstagramParser:
    """
    Parse les exports Instagram DMs (messages.json ou message_1.json).
    Structure : { "messages": [ { "sender_name": str, "content": str, "timestamp_ms": int } ] }
    """
    ...
    def parse(self, file_path: Path, my_name: str) -> Tuple[List[Message], List[Message]]:
        ...
        for m in messages:
            ...
            # Filtrer les partages de médias
            ...
            # Instagram encode parfois les noms en latin1
            try:
...
class ConversationAnalyzer:
    """
    Orchestre l'importation, l'analyse et la mise à jour du profil.
    """

    PARSERS = {
        ".txt": WhatsAppParser,
        ".json": None,   # déterminé à la volée
    }
    ...
    def analyze_file(self, file_path: Path) -> dict:
        """
        Analyse un fichier de conversation et met à jour le profil.
    ...
    def _detect_source(self, file_path: Path) -> str:
        ...
        if "telegram" in name or "result" in name:
            return "telegram"
        if "instagram" in name or "message" in name:
            return "instagram"
        return "whatsapp"
    ...
    def _build_report(
        ...
        interlocutors = list({
            m.sender for m in all_msgs if m.sender.lower() != self.my_name.lower()
        })
    ...
    def _print_report(self, report: dict):
        if "error" in report:
            ...
            return
        print(
            f"  ✅ {report['my_messages']} / {report['total_messages']} messages "
...
if __name__ == "__main__":
    ...
    # Remplace "Motaz" par ton nom exact dans les exports
